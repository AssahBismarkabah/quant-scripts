...
On 2026-09-18, the user authorized the reopening and the observability gate was re-run on free data ([phase_b5_census.md](../research/niche-etf-nav/phase_b5_census.md)): a frozen 70-fund universe (SPDR US ETFs under 400M dollars, no leveraged/inverse), 25,320 fund-days of official SSGA NAV plus the published daily premium/discount column, zero fetch failures. Findings: 265 events above 100 bps (1.05 percent of fund-days), concentrated in EM/foreign-hours funds; 84.4 percent of events shrink the next official NAV day and 45.6 percent converge below 50 bps. This is the daily signature of stale NAV marks. Daily observability PASSED; the mechanism disposition is unchanged UNVERIFIABLE - NAV/TIMING pending an intraday ETF-vs-executable-basket test, currently blocked by the locked Databento account and TWS/Gateway not running.
 
On 2026-09-21, the intraday access route was exhausted and B5-01 is closed as UNVERIFIABLE - ACCESS. IB Gateway 10.51 was installed and connected (live, port 4001); historical TRADES flowed, proving the API path, but BID_ASK was refused for lack of a US equities market-data subscription. The cheapest sufficient feeds (USD 1.50/month per network; USD 4.50/month for full basket coverage) are gated behind IBKR's minimum-equity requirement for market-data activation, which the account does not meet. Databento remains locked (auth_account_locked). The mechanism test is documented as requiring synchronized intraday quotes the project cannot currently own. Reopen conditions: IBKR equity above the market-data minimum with Network feeds active, an unlocked Databento account, or another owned zero-cost NBBO-class source; reopening additionally requires a re-frozen universe and observability re-run.
 
On 2026-09-21, the B15 spin-off family also closed ([s04_policy_closure.md](../research/spin-offs/s04_policy_closure.md)). The S04 subtype - index-mandated ex-date selling of a spun-off company - was eliminated by S&P DJI's September 2017 zero-price spin-off policy: the spinco is added to the parent's indices at zero price and removed at the end of the first trading day unless retained by announced committee decision, so index funds never receive the spinco outside the index. Confirmed in all four 2025-2026 large spincos (SNDK addition to SmallCap 600; SOLS S&P 500 add then announced migration to SmallCap 600 in the same rebalance; Q and HONA S&P 500 additions). Combined with the absence of any recipient-constraint evidence in the reviewed primary documents and the Abarbanell-Bushee-Raedy (2006) result that post-spin-off institutional rebalancing is generally not associated with abnormal price movements, the family terminates as UNVERIFIABLE - NO CONSTRAINT EVIDENCE. Reopen requires a new index policy or a newly reviewed constraint-bearing document class plus a re-frozen census.

### Memecoin launch-filter edge (registered, pre-research)

On 2026-09-21, a new family was registered as a pre-research specification: the memecoin launch-filter edge ([research-specs/memecoin-launch-filter-research-spec.md](../research-specs/memecoin-launch-filter-research-spec.md)). This is a new lane (on-chain launchpad token selection on Solana/pump.fun), not a resurfacing of any closed family; the on-chain valuation lane (Bitcoin MVRV) tested a different mechanism (holder-valuation timing, not launch selection). The registration includes: the verified landscape with sourced base rates (98.6% rug/pump-and-dump classification, under 2% graduation rate, ~0.4% of wallets ever realizing over $10k, ~100s median hold time), a frozen two-layer filter definition (rug-risk features from the Mazorra et al. 2022 feature set + structure/momentum criteria), a step-by-step procedure, and a pre-registered bootstrap gate. Status is analysis phase only: no data pull, no backtest, no live trade is authorized. Step 0 (open data-source feasibility searches, spec Section 9) is the only active work. Any advance beyond Step 0 requires completing Step 0 and running the pre-registered gate as written.
