...
# residualize hedge_norm within IV quintile (demean by IV bucket)
...
print(f'corr(hres, ret_next) = {np.corrcoef(m.hres, m.ret_next)[0,1]:.4f}')

print()
print('=== Obs 3 net-of-IV: hedge_norm residual -> fwd5 (naive) ===')
q = pd.qcut(m.hres.rank(method='first'), 5, labels=False)
means = m.assign(q=q).groupby('q').ret_fwd5.mean()*1e4
print('buckets ' + ' '.join(f'{v:+.1f}' for v in means.values) + f' | spread={(means.iloc[4]-means.iloc[0]):+.1f} bps')
means = m.assign(q=q).groupby('q').ret_next.mean()*1e4
print('next:    ' + ' '.join(f'{v:+.1f}' for v in means.values) + f' | spread={(means.iloc[4]-means.iloc[0]):+.1f} bps')
...
for lab, sub in [('IS', m[m.date<cut]), ('OOS', m[m.date>=cut])]:
    ...
    print(f'{lab} fwd5: buckets ' + ' '.join(f'{v:+.1f}' for v in means.values) + f' | spread={(means.iloc[4]-means.iloc[0]):+.1f} bps')

print()
print('=== Obs 4: skew -> returns ===')
