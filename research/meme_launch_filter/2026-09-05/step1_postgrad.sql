-- Post-graduation (PumpSwap AMM) trade aggregates for tokens that graduated 2026-09-05.
...
-- All 2026-09-05 graduates' pools were created in the Sep block month partition.
...
WITH grads AS (
  ...
  WHERE evt_block_date = DATE '2026-09-05'
  GROUP BY mint
),
pools AS (
  SELECT base_mint AS mint, pool, MIN(evt_block_time) AS pool_create_time
  FROM pumpdotfun_solana.pump_amm_evt_createpoolevent
  WHERE evt_block_date BETWEEN DATE '2026-09-05' AND DATE '2026-09-05' + INTERVAL '7' DAY
...
buy_sides AS (
  ...
  WHERE b.evt_block_date >= DATE '2026-09-05'
...
sell_sides AS (
  ...
  WHERE s.evt_block_date >= DATE '2026-09-05'
