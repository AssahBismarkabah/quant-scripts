-- First-hour trade aggregates per token created 2026-09-21, split creator vs non-creator
WITH launches AS (
  SELECT account_mint AS mint, call_block_time AS create_time, creator
  FROM pumpdotfun_solana.pump_call_create_v2
  WHERE call_block_date = DATE '2026-09-21'
  UNION ALL
...
trade_sides AS (
  ...
  WHERE t.block_time >= TIMESTAMP '2026-09-21 00:00:00'
    AND t.block_time < DATE '2026-09-21' + INTERVAL '1' DAY
