...
- **2026-08-15 (Stage-2 candidate closure at zero cost):** dealer-gamma paid-lane candidate (memo §6 option-a list, "options OI/dealer gamma") was tested **free** via the GitHub LambdaClass SPY options dataset (24.7M per-strike rows 2008-2025, OI 100% populated, full greeks — SHA256-verified, MIT). RESULT: DEAD (§8.10 in data-inventory-stage1.md) — predicts forward realized vol (corr +0.47, monotone) but not returns (flat/non-monotone, IS spread 0.0); monetizing it = short-vol = the closed VRP family. The OCC historical per-strike OI hunt (Series Search current-only, Daily OI aggregate-only) is also closed (§8.9). **Status of the three Stage-2 moat candidates: order-flow = DEAD (tested), dealer-gamma = DEAD (tested free), IVAMR long-intraday = the only one remaining untested (requires paid data).** Per edge-discovery-direction.md §7 the correct next action is the free-data **derive pass** (Stage 1) on data we already own — it has never been run.
