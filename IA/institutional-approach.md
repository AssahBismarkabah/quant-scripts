...
**Data moat (recorded 2026-08-13, see IA/edge-discovery-direction.md):** the process is necessary but not enough, AND the derive method runs identically on any data — **free data included.** What was measured-dead on free data was the specific set of public pre-specified anomalies we tested, NOT the derive-from-data method (which we have not yet run on any data). **Correct sequencing: exhaust the free-data derive pass first (Stage 1, cost 0); only after that is genuinely mined out do we buy a data moat (Stage 2) — and paid data only matters because it provides an asymmetry/moat, not a better method or process.** Data (free or paid) is necessary but not sufficient: it only becomes an edge after the derive-from-data + objective-rule + validate steps. If we neither exhaust free derivation nor buy a moat nor derive an objective edge, the disciplined answer is to not deploy (capital preservation), not to trade discretion.
