...
- Next: (a) move down the 9.0/8.0 candidates (e.g. JRFM "Deep Hedging Under Market Frictions", "AI and Exchange Rate Predictability", "Same Dollar, Different Impact", "How Likely/How Deep" crash bounds), (b) assess each on forced-counterparty / free-data / non-decayed before advance-to-spec or bin.
...
- **Pattern note (5 candidates so far):** HK = real effect, killed by friction (stamp duty). VIX-ETP = gross Sharpe 1.47 but naive trade killed by 1 tick; only heavy L2 cost-threshold build survives. Same Dollar = no tradable signal (effect is "no effect"). AI-FX = passes friction + free data easily (best profile yet) but weak forced-counterparty and heavy decay/crowding risk, with its own sample ending at the AI knowledge cutoff (no true OOS). The funnel consistently surfaces real, well-identified frontier effects whose executable layer is either friction-eaten, absent, or (now) decay-prone in the AI era — consistent with prior findings that free-data, high-prior, non-decayed trades are scarce. Remaining 9.0s: "Deep Hedging Under Market Frictions" (DRL options-hedging method), "The Trillion Dollar Bonus" (compensation, descriptive), "The hidden costs of hedge fund activism" (EJF, DiD liquidity), "How Likely and How Deep?" (crash bounds, theory/method).
