...
- **Pattern note (4 candidates so far):** HK = real effect, killed by friction (stamp duty). VIX-ETP = gross Sharpe 1.47 but naive trade killed by 1 tick; only heavy L2 cost-threshold build survives. Same Dollar = no tradable signal (effect is "no effect"). The funnel consistently surfaces real, well-identified frontier effects whose executable layer is either friction-eaten or absent — consistent with prior findings that free-data, high-prior, non-decayed trades are scarce. Remaining 9.0s: "AI and Exchange Rate Predictability" (Sharpe>0.7 FX strategy — favorable single-instrument friction), "Deep Hedging Under Market Frictions" (DRL options-hedging method), "The Trillion Dollar Bonus" (compensation, descriptive).
